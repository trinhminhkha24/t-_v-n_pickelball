...
> Ghi chú: Source đầy đủ của hệ Motion Compare đã được đặt ở root repo:
> `server/`, `motion-compare/`.
> README cập nhật tương ứng: `server/README.md`, `motion-compare/README.md`.

FastAPI + WebSocket server làm backend cho Motion Compare.
...
chạy server (hoặc dùng demo preset):

```bash
# Build from video (if you have sample videos)
python motion-compare/build_reference.py --video input-videos/Forehand-Drive.mp4

# OR use the included demo preset generator:
python motion-compare/create_demo_preset.py
```
